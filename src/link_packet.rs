use crate::{error::Result, keypair::Keypair};
use helium_proto::{
...
use semtech_udp::{pull_resp, push_data, MacAddress, StringOrNum};
...
impl LinkPacket {
    pub fn from_push_data(push_data: &push_data::RxPk, gateway_mac: MacAddress) -> Result<Self> {
        let payload = base64::decode(push_data.get_data())?;
        let packet = LoraPacket {
            ...
            routing: mk_routing_information(&payload)?,
            payload,
            rx2_window: None,
            oui: 0,
        };
    ...
    pub fn to_pull_resp(&self, use_rx2: bool) -> Option<pull_resp::TxPk> {
        let (timestamp, frequency, datarate) = if use_rx2 {
            if let Some(rx2) = &self.packet.rx2_window {
                (Some(rx2.timestamp), rx2.frequency, rx2.datarate.clone())
            } else {
                return None;
            }
        } else {
        ...
        Some(pull_resp::TxPk {
            ...
            data: base64::encode(&self.packet.payload),
